...
class KBHit:
    ...
    def kbhit(self):
        """ Returns True if keypress is waiting to be read. """
        if os.name == 'nt':
            return msvcrt.kbhit()
        else:
...
async def rover_keyboard_loop(rover: projectairsim.Rover):
    ...
    # Initialize the cross-platform keyboard handler
    kb = KBHit()
    ...
    while running:
        if kb.kbhit():
            ...
            # Convert to string for comparison (Linux returns str, Windows bytes)
            # We normalize everything to lower-case string for easier comparison
            if isinstance(c, bytes):
                key = c.decode('utf-8').lower()
            else:
                key = c.lower()
            ...
            if key == 'w':
                ...
                running = False
                # Restore terminal settings on Linux before exiting loop
                if os.name != 'nt':
                    kb.set_normal_term()
                continue
